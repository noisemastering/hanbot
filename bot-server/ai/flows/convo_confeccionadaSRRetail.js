// ai/flows/convo_confeccionadaSRRetail.js
// Convo flow for malla sombra confeccionada SIN REFUERZO — retail, buyer.
// Assembled from: master_flow + product_flow + retail_flow + buyer_flow
// Handles non-promo retail buyers who want confeccionada sin refuerzo (rectangular only).
//
// Product: Malla sombra Raschel 90%, PEAD con protección UV, costura reforzada
// y argollas cada 80 cm. Sin refuerzo en esquinas (a diferencia de la reforzada).
...
const manifest = {
  type: 'convo_flow',
  name: 'convo_confeccionadaSRRetail',
  products: ['69431347ed2d4185ba4706c2'],  // Confeccionada sin Refuerzo — Rectangular (5 sizes)
  clientProfile: 'buyer',
  salesChannel: 'retail',
  endpointOfSale: 'online_store',
  voice: 'casual',
  installationNote: 'La malla viene lista para instalar con costura reforzada y argollas cada 80 cm para máxima seguridad. Solo se necesita soga o cable para sujetarla. Fabricada en polietileno de alta densidad (PEAD) con protección UV, bloquea hasta el 90% del sol. Etiqueta oficial Hanlob, 100% hecha en México.',
  allowListing: false,
  offersCatalog: false,
  promo: null
};
...
async function handle(userMessage, convo, psid, state = {}) {
  ...
  if (dims) {
    ...
    if (hasFractions) {
      ...
      console.log(`📏 [confeccionadaSRRetail] Fractional ${w}x${h}m → offering ${rw}x${rh}m`);
    ...
    // productFlow will check other convo_flows (e.g. reforzada) and offer a switch
    // if the size exists there, instead of going straight to handoff.
    console.log(`📏 [confeccionadaSRRetail] Size ${w}x${h}m not in sin-refuerzo catalog — falling through to chain`);
  }
