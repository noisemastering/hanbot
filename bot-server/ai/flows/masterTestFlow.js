...
async function handle(classification, sourceContext, convo, psid, campaign = null, userMessage = '') {
  ...
  if (!hasNonMetricUnit) {
    if (PURE_DIMENSION_MSG.test(cleanForFullMatch)) {
      ...
      // Message contains dimensions + extra content
      // If the extra content is just a price question or dimension labels (largo/ancho), trust the regex parser
      const stripped = cleanForFullMatch.replace(/\d+(?:[.,]\d+)?\s*(?:m(?:trs?|ts|etros?|t)?\.?)?\s*(?:d(?:e)?\s*)?(?:largo|ancho)\s*/gi, '')
        .replace(/(?:x|×|por)\s*/gi, '')
        .replace(/(?:cu[aá]nto|cuanto)\s+(?:cuesta|sale|vale|est[aá]|queda)/gi, '')
        .replace(/(?:precio|costo|cotiza)/gi, '')
        .replace(/[?,.\s]/g, '').trim();
      if (stripped.length === 0) {
        // Pure dimension + price question with largo/ancho labels — regex handles it
        const dimsFromMessage = parseDimensions(userMessage);
        if (dimsFromMessage) {
          console.log(`🌐 Malla flow - [REGEX] Dimension+price with largo/ancho: "${userMessage.slice(0, 40)}" → ${dimsFromMessage.width}x${dimsFromMessage.height}`);
          state.width = dimsFromMessage.width;
          state.height = dimsFromMessage.height;
          state.userExpressedSize = dimsFromMessage.userExpressed;
        }
  ...
  if (state.width && state.height && Math.max(state.width, state.height) > 12) {
    ...
    const rollSize = `${minSide}x${maxSide}`;

    // Both sides > 8m → custom order, hand off to specialist immediately
    if (minSide > 8 && maxSide > 8) {
      console.log(`🏭 Oversize detected early: ${rollSize}m (both sides >8m) — handoff`);
      return await executeHandoff(psid, convo, userMessage, {
        reason: `Medida grande: ${rollSize}m (ambos lados >8m)`,
        responsePrefix: `Esa medida (${rollSize}m) requiere cotización especial ya que es más grande que nuestro catálogo estándar. Te comunico con un especialista para cotizarte.`,
        lastIntent: 'custom_order_handoff',
        extraState: { customOrderSize: `${rollSize}m` },
        timingStyle: 'none'
      });
    }
