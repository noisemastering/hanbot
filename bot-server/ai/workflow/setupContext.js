  ...
  const pushDefault = async (prod, pi, promoLabel) => {
    ...
    lines.push(D(
      `- ${promoLabel ? `PROMOCIÓN destacada: "${promoLabel}" → ` : "Medida destacada (por defecto): "}` +
        ...
        `PERO un SALUDO o charla SIN señal de compra ni medida ("buenas tardes", "hola", "buenos días", "qué tal", "buenas") NO es interés de compra: contesta SOLO con un saludo humano y breve tipo "¡Hola! Soy X de Hanlob, ¿en qué te puedo ayudar?" — NO menciones la promoción, ni la medida, ni el precio, ni el link. Eso llega DESPUÉS, SOLO cuando el cliente pida precio/info o muestre interés. ` +
