...
const GENDER_COLORS = { blue: '#3B82F6', pink: '#EC4899' };
          ...
          {geoData.length > 0 && (
            <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-6">
              ...
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={geoData.slice(0, 8).map(g => ({ name: abbrState(g.state), value: g.count, percentage: g.percentage }))}
                      cx="50%" cy="50%" innerRadius={50} outerRadius={85} paddingAngle={3} dataKey="value"
                      label={({ name, percentage }) => `${name}: ${percentage}%`}>
                      {geoData.slice(0, 8).map((_, i) => (<Cell key={i} fill={AD_COLORS[i % AD_COLORS.length]} stroke="transparent" />))}
                    </Pie>
          ...
          {genderData.length > 0 && (
            <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-6">
              ...
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={genderData.map(g => ({ name: g.gender === 'male' ? 'Hombres' : g.gender === 'female' ? 'Mujeres' : 'Desconocido', value: g.count, percentage: g.percentage }))}
                      cx="50%" cy="50%" innerRadius={50} outerRadius={85} paddingAngle={4} dataKey="value"
                      label={({ name, percentage }) => `${name}: ${percentage}%`}>
                      {genderData.map((g, i) => (<Cell key={i} fill={g.gender === 'male' ? GENDER_COLORS.blue : g.gender === 'female' ? GENDER_COLORS.pink : '#6B7280'} stroke="transparent" />))}
                    </Pie>
