    ...
    <div>
      ...
      <div className="bg-gray-800/50 backdrop-blur-lg border border-gray-700/50 rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-700/50">
          <h2 className="text-xl font-bold text-white">{t('usos.listTitle')}</h2>
        </div>

        {loading ? (
          <div className="p-8 text-center">
            <div className="inline-block w-8 h-8 border-4 border-primary-500 border-t-transparent rounded-full animate-spin"></div>
            <p className="text-gray-400 mt-4">{t('usos.loadingUsos')}</p>
          </div>
        ) : usos.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-white mb-2">{t('usos.noUsosRegistered')}</h3>
            <p className="text-gray-400 mb-6">{t('usos.startAdding')}</p>
            <button
              onClick={onAdd}
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        {!uso.available && (
                          <span className="px-2 py-0.5 bg-red-500/20 text-red-300 rounded text-xs font-medium">
                            {t('usos.inactiveLabel')}
                          </span>
                        )}
                      </div>
                      {uso.products && uso.products.length > 0 && (
                        <div className="text-xs text-gray-500 mt-1">
                          {(uso.productCount || uso.products.length) !== 1
                            ? t('usos.productsCountPlural', { count: uso.productCount || uso.products.length })
                            : t('usos.productsCount', { count: uso.productCount || uso.products.length })}
                        </div>
                      )}
                    </td>
                    <td className="px-6 py-4">
                        ...
                        if (populatedProducts.length === 0 && unpopulatedCount > 0) {
                          return (
                            <div className="mt-2 text-xs text-yellow-500">
                              {unpopulatedCount !== 1
                                ? t('usos.notFoundPlural', { count: unpopulatedCount })
                                : t('usos.notFound', { count: unpopulatedCount })}
                            </div>
                          );
                        }
                          ...
                          <div className="mt-2 flex flex-wrap gap-1">
                            ...
                            {populatedProducts.length > 3 && (
                              <span className="px-2 py-0.5 bg-gray-700/50 text-gray-400 rounded text-xs">
                                {t('usos.more', { count: populatedProducts.length - 3 })}
                              </span>
                            )}
                            {unpopulatedCount > 0 && (
