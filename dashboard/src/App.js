...
const menuItems = [
  ...
  },
  {
    ...
    children: [
      // ── Administración ──
      { id: "label-admin", labelKey: "menu.campAdmin", isLabel: true },
      {
        id: "campaigns",
      ...
      // ── Análisis ──
      { id: "label-analysis", labelKey: "menu.campAnalysis", isLabel: true },
      {
        id: "ad-performance",
        labelKey: "menu.desempeno",
        path: "/ad-performance",
        icon: (
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
        )
  ...
  },
  {
    ...
    children: [
      ...
      {
        id: "playground-simulador",
        labelKey: "menu.simulador",
      ...
      {
        id: "playground-rendimiento",
        labelKey: "menu.messagePerformance",
...
function App() {
  ...
  const filteredMenuItems = menuItems.filter(item => {
    ...
    if (item.children && item.children.length > 0) {
      return item.children.some(child => !child.isLabel && canAccess(child.id));
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
                    ...
                    {isExpanded && (
                      <div className="ml-4 space-y-1 border-l-2 border-gray-700/50 pl-2">
                        {item.children.map((child) => (
                          child.isLabel ? (
                            <p key={child.id} className="text-[10px] uppercase tracking-wider text-gray-600 font-semibold px-4 pt-3 pb-1">{t(child.labelKey)}</p>
                          ) : (
                          <NavLink
                            key={child.id}
                            to={child.path}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Routes>
          ...
          <Route path="/sales-overview" element={<SalesOverviewView />} />
          ...
          <Route path="/consumo" element={canAccess('consumo') ? <ConsumoView /> : <Navigate to={getLandingPage()} replace />} />
          <Route path="/playground/mapa" element={effectiveRole === 'super_admin' ? <MapaView /> : <Navigate to={getLandingPage()} replace />} />
          ...
          <Route path="/crm/sales" element={<CRMSalesView />} />
